...
async function identifySong() {
    ...
    await $`ffmpeg -y -i ../Music/stream/last_six_seconds.mp3 -f chromaprint ../Music/stream/current_fingerprint.txt`
    const current_fingerprint = await Bun.file("../Music/stream/current_fingerprint.txt").text();
    const acoustid_url = `https://api.acoustid.org/v2/lookup?client=${Bun.env.ACOUSTID_CLIENT_KEY}&duration=12&fingerprint=${current_fingerprint}`
